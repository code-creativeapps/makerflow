...
const Planning = ({ planning, isDragging }) => {
    ...
    const renderTasks = (date) => {
        ...
        if (dayFound) {
            ...
            return <p className="text-xs text-gray-300 cursor-default">Drop some tasks here...</p>
        }
    }
    return (
        <>
        {/* Planning */}
            ...
            <div className={`relative w-full px-4 pt-4 pb-10 ${layout == 'column' ? 'overflow-hidden' : 'overflow-y-scroll'} border border-gray-300`}>
                ...
                <div class="flex">
                    ...
                    <div class="flex">
                        <div onClick={() => changeWeekView('backward')} class={`hover:bg-gray-600 hover:text-white bg-white text-gray-600 py-2 px-3 text-xs rounded-l flex items-center cursor-pointer`}>
                            <svg class="w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                            </svg>
                        </div>
                        <div onClick={() => changeWeekView('forward')} class={`hover:bg-gray-600 hover:text-white bg-white text-gray-600 py-2 px-3 text-xs border-gray-200 rounded-r flex items-center cursor-pointer`}>
                            <svg class="w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <Droppable key={i} droppableId={moment(date).format('DD/MM/YYYY')}>
                            {(provided, snapshot) => {
                                ...
                                <div className={
                                    ...
                                    <h3 className={`flex text-lg font-medium text-gray-700 ${layout == 'column' ? 'flex-col text-left': 'items-center'}`}>{moment(date).format('dddd')} <span className={`${layout == 'row' ? 'ml-2' : 'mb-3'} text-xs font-normal`}>{moment(date).format('LL')}</span></h3>
                                    <div {...provided.droppableProps} innerRef={provided.innerRef} ref={provided.innerRef} className={
                                        ...
                                        { renderTasks(date) }
                                        {/* <div className="p-2 my-2 text-xs font-semibold text-white bg-pink-400 rounded shadow-sm">Write UX/UI Brief</div>
                                        <div className="p-2 my-2 text-xs font-semibold text-white bg-purple-400 rounded shadow-sm">Write UX/UI Brief</div>
                                        <div className="p-2 my-2 text-xs font-semibold text-white bg-green-400 rounded shadow-sm">Write UX/UI Brief</div>
                                        <div className="flex justify-between p-2 my-2 text-xs font-semibold text-white bg-pink-600 rounded shadow-sm">Write UX/UI Brief
                                            <div className="flex">
