...
import { _deleteProject, _addMilestone, _setProjectColor, _updateProject } from './actions'
...
const Project = ({project}) => {
	...
	const handleKeyDown = (event) => {
		if (event.key === 'Enter') {
			dispatch(_updateProject({name: event.target.value}, project.id))
	...
	const handleKeyDownMilestone = (event) => {
		if (event.key === 'Enter') {
		  	dispatch(_addMilestone(event.target.value, project.id))
			event.target.value = ''
			return
		}
	  }
	...
	const changeColor = (color) => {
		...
		dispatch(_updateProject({color}, project.id))
			...
			<div className="flex items-center justify-between mb-4 group">
				...
				{ !edit && (
					<div class="flex cursor-pointer">
						...
						<svg onClick={() => { if (window.confirm('Are you sure you want to delete this project?')) dispatch(_deleteProject(project.id))}} className="w-5 text-gray-600 opacity-0 group-hover:opacity-100" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
